...
export default function Home() {
  ...
  const handleAdicionarEvento = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const eventosAtualizados: EventoCalendario[] = await eventoResponse.json();
  ...
  const handleEditEvento = (id: number) => {
    ...
    if (evento) {
      const data = evento.start instanceof Date ? evento.start.toISOString().slice(0, 16) : new Date(evento.start).toISOString().slice(0, 16);

      setFormEvento({
        titulo: evento.title as string,
        descricao: (evento as CustomEvent).desc || "",
        data,
        idEdicao: id,
      });
  ...
  const handleDeleteEvento = async (id: number) => {
    ...
    try {
      const response = await fetch(API_URL + `evento_calendario/${id}`, {
        method: "DELETE",
      });
    ...
    <>
      <div className='flex flex-col w-[90%] md:w-[85%] py-12 items-center gap-10'>
        <div className='flex flex-col w-full gap-5'>
          ...
          <div className='flex flex-col md:flex-row w-full gap-5 text-sm md:text-base lg:text-lg'>
            <div className='w-full md:w-3/5 px-5 md:px-8 py-2.5 md:py-4 bg-surface-primary dark:bg-surface-secondary rounded transition-all duration-400'>
              <DnDCalendar
            ...
            <div className='flex flex-col w-full md:w-2/5 gap-5'>
              <div className='w-full grow px-5 md:px-8 py-2.5 md:py-4 bg-surface-primary dark:bg-surface-secondary rounded transition-all duration-400'>
                    ...
                    events.map((event) => (
                      <div key={event.id} className='p-3 bg-background rounded border-l-4 border-brand transition-all duration-400'>
                        <div className='flex justify-between items-start gap-2 mb-2'>
                          ...
                          <button onClick={() => handleDeleteEvento(event.id as number)} className='text-error-500 hover:text-error-600 focus:text-error-700'>
                            <svg viewBox='0 0 24 24' fill='none' stroke='currentColor' strokeWidth='2' strokeLinecap='round' className='h-5'>
                              <path d='M18 6L6 18M6 6L18 18' />
                            </svg>
                          </button>
                        </div>
                        {(event as CustomEvent).desc && <p className='text-xs text-secondary mb-2'>{(event as CustomEvent).desc}</p>}
                        <button
                          onClick={() => handleEditEvento(event.id as number)}
                          className='text-xs px-2 py-1 bg-brand hover:bg-brand-hover focus:bg-brand-focus rounded text-primary font-semibold'>
                          Editar
                        </button>
                      </div>
                    ))
                ...
                <div>
                  <button
                    ...
                    className='w-full px-3 py-1.5 bg-brand hover:bg-brand-hover focus:bg-brand-focus rounded text-primary text-base font-semibold cursor-pointer'>
                  ...
                  {mostrarFormEvento && (
                    <div className='flex flex-col gap-3 pt-3'>
                      ...
                      <button
                        ...
                        className={`px-3 py-1.5 rounded text-primary text-base font-semibold cursor-pointer ${
              ...
              <div className='w-full px-5 md:px-8 py-2.5 md:py-4 bg-surface-primary dark:bg-surface-secondary rounded transition-all duration-400'>
                <h3 className='text-lg md:text-xl font-semibold mb-3'>Sua Rotina Semanal</h3>
                {rotinaSemanal ? (
        ...
        <div className='flex flex-col w-full gap-5'>
          ...
          <div className='flex flex-col w-full gap-8 px-5 md:px-10 py-2.5 md:py-5 bg-surface-primary dark:bg-surface-secondary rounded transition-all duration-400'>
            ...
            <button
              ...
              className={`flex w-fit px-3 py-1.5 gap-3 items-center rounded text-lg font-semibold cursor-pointer ${
